<script lang="ts">
	...
	let localValue = $state<DateValue | undefined>();
	$effect(() => {
		if (localValue) {
			value = new Date(localValue.toDate('UTC'));
		}
	});
</script>

<DatePicker.Root weekdayFormat="short" fixedWeeks={true} bind:value={localValue}>
	<div class="flex w-full max-w-[232px] flex-col gap-1.5">
		<DatePicker.Input
			class="h-input rounded-input border-border-input bg-background text-foreground focus-within:border-border-input-hover focus-within:shadow-date-field-focus hover:border-border-input-hover flex w-full max-w-[232px] select-none items-center border px-2 py-3 text-sm tracking-[0.01em] rounded-lg"
			{...props}
		>
				...
				{#each segments as { part, value }}
					<div class="inline-block select-none">
						...
						{:else}
							<DatePicker.Segment
								{part}
								class="rounded-5px hover:bg-muted focus:bg-muted focus:text-foreground aria-[valuetext=Empty]:text-muted-foreground focus-visible:ring-0! focus-visible:ring-offset-0! px-1 py-1 rounded-sm"
							>
								{value}
							</DatePicker.Segment>
						{/if}
		...
		<DatePicker.Content sideOffset={6} class="z-50 bg-background">
			<DatePicker.Calendar
